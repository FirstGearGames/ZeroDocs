 ...
 * and lays that out into a PDF with a title page, a page-numbered contents
 * list, and a real PDF outline (bookmarks) - the reader's own left-hand
 * navigation panel, nested to match the pages' folder structure so it isn't
 * one flat wall of 200+ entries; every group starts collapsed.
 *
 ...
 * overflow the page.
 *
 * Links are clickable: absolute http(s) URLs become URI annotations, and
 * internal Markdown links (`[text](../other-page)`, `/docs/v1/x`, `#anchor`)
 * resolve to the target page's slug and become GoTo jumps to where that page
 * starts in the PDF. The contents list is clickable the same way.
 *
 * ponytail: a GoTo lands on the target page's first PDF page, not the exact
 * heading an `#anchor` names - one destination per doc page, no per-heading
 * destination map.
 */
...
class Doc {
  constructor() {
    this.pages = []; // { ops: string, annots: [{ rect:[x0,y0,x1,y1], url }] or [{ rect, slug }] }
  ...
  drawLine(tokens, x0, size, opts = {}) {
    ...
    for (const tok of tokens) {
      ...
      if (tok.style.url) {
        ...
        const rect = [x, y - 2, x + w, y + size];
        if (/^https?:\/\//i.test(href)) this.page.annots.push({ rect, url: href });
        else {
          const slug = this.resolveSlug ? this.resolveSlug(tok.style.url) : null;
          if (slug !== null) this.page.annots.push({ rect, slug });
        }
...
/** The directory internal links on `page` resolve against (its own folder for an index page, its parent otherwise). */
function linkBaseDir(page) {
  if (/(^|[\\/])index\.md$/i.test(page.file || '')) return page.slug;
  const cut = page.slug.lastIndexOf('/');
  return cut < 0 ? '' : page.slug.slice(0, cut);
}

/** A Markdown link href -> a known page slug in this version, or null when it points nowhere. */
function internalSlug(url, baseDir, slugs) {
  const target = String(url).split('#')[0].replace(/\\/g, '/');
  if (!target) return slugs.has(baseDir) ? baseDir : null; // bare "#anchor": this same page
  let clean = target.replace(/\.md$/i, '').replace(/\/$/, '');
  if (clean.startsWith('/')) clean = clean.replace(/^\/(?:docs\/)?/, '');
  else clean = path.posix.normalize(path.posix.join(baseDir, clean));
  clean = clean.replace(/^\.?\//, '').replace(/\/index$/, '');
  if (clean === '.' || clean === 'index') clean = ''; // the version's own landing page
  if (slugs.has(clean)) return clean;
  // A root-relative link carries the version ("/docs/v1/core-api"); slugs do not.
  const withoutVersion = clean.replace(/^[^/]+\//, '');
  return slugs.has(withoutVersion) ? withoutVersion : null;
}
...
async function renderVersionPdf(site, config, version, listPages) {
  ...
  doc.paragraph([{ text: `Generated ${new Date().toISOString().replace('T', ' ').slice(0, 19)} UTC` }]);
  ...
  for (const p of pages) {
    ...
    doc._startPage();
    const baseDir = linkBaseDir(p);
    doc.resolveSlug = url => internalSlug(url, baseDir, slugs);
    layoutPage(doc, p.title, p.blocks, images);
    toc.push({ title: p.title, page: startPage, slug: p.slug });
  }
  doc.resolveSlug = null;
  doc.slugPages = new Map(toc.map(e => [e.slug, e.page]));
  ...
  for (const entry of toc) {
    ...
    doc.page.annots.push({ rect: [MARGIN, doc.y - SIZE.body * 1.1, PAGE_W - MARGIN, doc.y + 2], slug: entry.slug });
    doc.y -= TOC_LINE_H;
...
// ─────────────────────────────────────────────────────────────────────────────
// Bookmarks (PDF Outline): the reader's own left-hand navigation panel
// ─────────────────────────────────────────────────────────────────────────────

/** toc entries (title/page/slug) -> a tree of {title, page, children} following the pages' own folder nesting. */
function buildOutlineTree(tocEntries) {
  const root = { title: null, page: null, children: [] };
  for (const entry of tocEntries) {
    const parts = entry.slug ? entry.slug.split('/') : [];
    if (!parts.length) { root.title = entry.title; root.page = entry.page; continue; }
    let node = root;
    for (let i = 0; i < parts.length; i++) {
      let child = node.children.find(c => c.key === parts[i]);
      if (!child) { child = { key: parts[i], title: null, page: null, children: [] }; node.children.push(child); }
      if (i === parts.length - 1) { child.title = entry.title; child.page = entry.page; }
      node = child;
    }
  }
  // The version landing page becomes its own top entry, ahead of the folder groups.
  if (root.page != null) root.children.unshift({ title: root.title, page: root.page, children: [] });
  return root;
}

/** First page reachable from `node` (its own, else its first descendant's) - a folder header needs somewhere to jump to. */
function firstReachablePage(node) {
  if (node.page != null) return node.page;
  for (const c of node.children) { const p = firstReachablePage(c); if (p != null) return p; }
  return null;
}
...
/** Allocate + fill every outline (bookmark) object; returns the /Outlines object number, or null with no pages. */
function writeOutlines(w, root, pageObjNums) {
  if (!root.children.length) return null;
  const allNodes = [];
  (function collect(node) { for (const c of node.children) { allNodes.push(c); collect(c); } })(root);
  const numOf = new Map(allNodes.map(n => [n, w.reserve()]));

  function fill(list, parentObjNum) {
    list.forEach((node, i) => {
      const dest = firstReachablePage(node);
      const pageObjNum = dest != null ? pageObjNums[dest - 1] : null;
      let body = `<< /Title (${pdfEscape(node.title || 'Untitled')}) /Parent ${parentObjNum} 0 R`;
      if (i > 0) body += ` /Prev ${numOf.get(list[i - 1])} 0 R`;
      if (i < list.length - 1) body += ` /Next ${numOf.get(list[i + 1])} 0 R`;
      if (node.children.length) {
        body += ` /First ${numOf.get(node.children[0])} 0 R /Last ${numOf.get(node.children[node.children.length - 1])} 0 R`
          + ` /Count ${-countDescendants(node)}`; // negative: collapsed by default, so 200+ pages doesn't open as one wall of entries
      }
      if (pageObjNum != null) body += ` /Dest [${pageObjNum} 0 R /Fit]`;
      w.set(numOf.get(node), body + ' >>');
      if (node.children.length) fill(node.children, numOf.get(node));
    });
  }

  const outlinesObj = w.reserve();
  fill(root.children, outlinesObj);
  w.set(outlinesObj, `<< /Type /Outlines /First ${numOf.get(root.children[0])} 0 R `
    + `/Last ${numOf.get(root.children[root.children.length - 1])} 0 R /Count ${root.children.length} >>`);
  return outlinesObj;
}
...
function assemble(doc) {
  ...
  // Page objects are reserved up front: a link on page 3 can jump to page
  // 200, so every page's object number has to exist before any annotation
  // referencing it is written.
  const pageObjNums = doc.pages.map(() => w.reserve());
  const slugPages = doc.slugPages || new Map();
  doc.pages.forEach((pg, idx) => {
    const stream = pg.ops.join('\n');
    const streamObj = w.add(`<< /Length ${Buffer.byteLength(stream, 'latin1')} >>\nstream\n${stream}\nendstream`);
    const annotNums = [];
    for (const a of pg.annots) {
      const common = `<< /Type /Annot /Subtype /Link /Rect [${a.rect.map(n => n.toFixed(2)).join(' ')}] /Border [0 0 0] `;
      if (a.url) {
        annotNums.push(w.add(common + `/A << /Type /Action /S /URI /URI (${pdfEscape(a.url)}) >> >>`));
        continue;
      }
      const targetPage = slugPages.get(a.slug);
      if (targetPage == null) continue;
      annotNums.push(w.add(common + `/Dest [${pageObjNums[targetPage - 1]} 0 R /Fit] >>`));
    }
    const annotsPart = annotNums.length ? ` /Annots [${annotNums.map(n => `${n} 0 R`).join(' ')}]` : '';
    w.set(pageObjNums[idx], `<< /Type /Page /Parent ${pagesRoot} 0 R /MediaBox [0 0 ${PAGE_W} ${PAGE_H}] `
  ...
  const outlinesObj = doc.outline ? writeOutlines(w, buildOutlineTree(doc.outline), pageObjNums) : null;
  const outlinePart = outlinesObj ? ` /Outlines ${outlinesObj} 0 R /PageMode /UseOutlines` : '';
  w.set(catalog, `<< /Type /Catalog /Pages ${pagesRoot} 0 R${outlinePart} >>`);
  return w.serialize();
}

module.exports = { renderVersionPdf, internalSlug, parseBlocks, wrapTokens, tokenize, textWidth, pdfEscape, PdfWriter };
