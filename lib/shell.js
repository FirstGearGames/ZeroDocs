...
// Must track `html.dark --bg` in assets/app.css.
const DARK_BG = '#0f1117';
const MIN_LINK_CONTRAST = 4.5; // WCAG AA for normal-size text

/**
 * A link color that stays legible on the dark background.
 *
 * Most sites set `theme.primary` and nothing else, and a primary picked for a
 * white page (the #2563eb default is 3.6:1 on #0f1117) is too dark to read as
 * a link in dark mode. Rather than drop the site's hue, this mixes it toward
 * white in 10% steps and stops at the first value clearing AA. A color that
 * already passes (a deliberately light `primaryDark`) is returned untouched,
 * and a non-hex color is passed through since it cannot be measured.
 */
...
<div class="header-inner">
  ...
  <nav class="top-nav" id="topnav" aria-label="Site links">
    ${(config.nav || []).map(n => `<a href="${escAttr(safeUrl(n.href || '#'))}"${/^https?:\/\//.test(n.href || '') ? ' target="_blank" rel="noopener"' : ''}>${escHtml(n.label || n.title || '')}</a>`).join('')}
  </nav>
  <div class="hright">
