...
function themeVars(theme) {
  ...
  return `:root{--link:${p};--act-bg:${p};--act-fg:${readableOn(p)};--sidebar:${sb};${callouts('.1')}}`
    + `html.dark{--link:${pDark};--act-bg:${pDark};--act-fg:${readableOn(pDark)};--sidebar:${sbDark};${callouts('.12')}}`;
}
 ...
 */
function renderShell({ config, versions, page, assetsVer, staticMode }) {
  ...
  const boot = JSON.stringify({ config, versions, staticMode: !!staticMode }).replace(/</g, '\\u003c');
...
<div class="header-inner">
  ...
  <a class="logo" href="/docs/${escAttr(versions[0] || 'v1')}">
    ${logoHtml(config)}
  ...
  <div class="hright">
    ...
    <select class="ver-sel" id="versel" aria-label="Documentation version">
